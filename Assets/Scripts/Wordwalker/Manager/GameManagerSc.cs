...
public class GameManagerSc : MonoBehaviour
{
    ...
    private static void handleCharacterUnlocks()
    {
        // lastly, handle character unlocks (if eligible)
        bool showUnlockAlert = false;

        switch (localDBcopy.group)
        {
            case "easy":
                showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-jesse", true);
                break;
            case "medium": 
                showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-jango", true);
                break;
            case "hard":
                showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-shadow-smitty", true);
                break;
            case "very-hard":
                if (localDBcopy.databaseId == "standard/nightmare")
                {
                    showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-nightingale", true);
                }
                else
                {
                    showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-monk", true);
                }
                break;
            case "trivia":
                ...
                {
                    showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-winter", true);
                }
                else if (localDBcopy.databaseId == "trivia/celebs" || localDBcopy.databaseId == "trivia/pokemon")
                {
                    showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-hazel", true);
                }
                break;
            default: break;
        }
        ...
        if (stars >= 12)
            showUnlockAlert = GlobalStatMap.AddOrModifyBoolReturnChanged("char-unlock-golden-smitty", true);

        if(showUnlockAlert)
        {
            GlobalStatMap.AddFlag("newCharUnlocked");
        }
    }
